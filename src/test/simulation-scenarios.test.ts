...
const makeEdge = (id: string, source: string, target: string, latency = 25): Edge => ({
  id,
  type: 'systemEdge',
  source,
  target,
...
describe('simulation scenarios', () => {
  it('marks all nodes in an outage region as failed', () => {
    ...
    expect(state.warnings.some((warning) => warning.type === 'failure' && warning.nodeId === 'a')).toBe(true);
  });
