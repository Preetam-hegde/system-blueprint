  ...
  runAnalysis: () => {
    ...
    nodes.forEach((n) => {
      ...
      if (backlogSet.has(n.id)) {
        warnings.push({
          ...
          severity:
            data.currentLoad > effectiveThroughputLimit
              ? "critical"
              : "warning",
        });
      }
      ...
      if (data.currentLoad > effectiveThroughputLimit) {
        warnings.push({
          ...
          severity:
            data.currentLoad > effectiveThroughputLimit * 1.5
              ? "critical"
              : "warning",
        });
      }
    ...
    sourceNodes.forEach((src) => {
      ...
      const dfs = (nodeId: string, cumLatency: number, path: string[]) => {
        ...
        const totalLatency =
          cumLatency + getReplayNodeLatency(nodeData, simulation);
        const outgoing = edges.filter((e) => e.source === nodeId);
        if (outgoing.length === 0 && totalLatency > 200) {
          const retryLabel =
            simulation.replayTrace.estimatedRetries > 0
