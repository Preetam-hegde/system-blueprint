...
export type SystemNodeType =
  | 'server' | 'api-gateway' | 'load-balancer' | 'cdn' | 'serverless' | 'job-worker' | 'scheduler'
  | 'sql-db' | 'nosql-db' | 'vector-db' | 'cache' | 'object-storage' | 'data-lake' | 'graph-db' | 'time-series-db'
  | 'message-queue' | 'event-bus' | 'stream-processor' | 'webhook' | 'dead-letter-queue'
  | 'llm' | 'rag-pipeline' | 'ml-model' | 'embedding-service' | 'ai-agent' | 'fine-tuning' | 'model-router'
  | 'dns' | 'firewall' | 'rate-limiter' | 'service-mesh' | 'reverse-proxy' | 'service-discovery'
  | 'web-client' | 'mobile-client' | 'iot-device' | 'desktop-client'
  | 'log-aggregator' | 'metrics-server' | 'tracing' | 'alerting' | 'error-tracker'
  | 'waf' | 'vault' | 'identity-provider' | 'oauth-server' | 'kms';
...
export type SimulationScenarioType = 'none' | 'zone-outage' | 'regional-latency' | 'queue-backlog';
...
export interface SavedSimulationPreset {
  ...
  simulation: Pick<
    SimulationState,
    'speed' | 'rps' | 'mode' | 'maxSteps' | 'packetLossPct' | 'retryAttempts' | 'retryBackoffMs' | 'extraLatencyMs' | 'manualFailedNodeIds' | 'scenario'
  >;
}
...
export const NODE_CATALOG: NodeCatalogItem[] = [
  ...
  { type: 'job-worker', label: 'Job Worker', category: 'compute', description: 'Async background worker', whatItDoes: 'Consumes queued work and processes tasks outside the user request path.', usedFor: ['Email and notification jobs', 'Image or document processing', 'Long-running background tasks'] },
  { type: 'scheduler', label: 'Scheduler', category: 'compute', description: 'Time-based job trigger', whatItDoes: 'Starts jobs or workflows on a schedule or at specific times.', usedFor: ['Cron-style tasks', 'Nightly batch jobs', 'Periodic syncs and cleanups'] },
  // Storage
  ...
  { type: 'time-series-db', label: 'Time-Series DB', category: 'storage', description: 'Timestamped metric storage', whatItDoes: 'Stores time-indexed measurements efficiently for querying trends and recent values.', usedFor: ['Metrics retention', 'IoT telemetry history', 'Operational trend analysis'] },
  // Messaging
  ...
  { type: 'dead-letter-queue', label: 'Dead Letter Queue', category: 'messaging', description: 'Failed message holding queue', whatItDoes: 'Captures messages or jobs that could not be processed successfully after retries.', usedFor: ['Failure isolation', 'Poison message handling', 'Operational replay and debugging'] },
  // AI / ML
  ...
  { type: 'model-router', label: 'Model Router', category: 'ai-ml', description: 'Routes requests across models', whatItDoes: 'Chooses which model or provider should handle a request based on cost, latency, or capability rules.', usedFor: ['Multi-model systems', 'Fallback and failover routing', 'Cost-aware inference selection'] },
  // Networking
  ...
  { type: 'service-discovery', label: 'Service Discovery', category: 'networking', description: 'Tracks service endpoints', whatItDoes: 'Maintains a registry of healthy service instances so clients can find them dynamically.', usedFor: ['Microservice endpoint lookup', 'Dynamic scaling environments', 'Internal name resolution'] },
  // Clients
  ...
  { type: 'error-tracker', label: 'Error Tracker', category: 'observability', description: 'Captures application exceptions', whatItDoes: 'Collects runtime exceptions, stack traces, and release context for faster debugging.', usedFor: ['Frontend and backend error monitoring', 'Release regression tracking', 'Crash triage'] },
  // Security
  ...
  { type: 'kms', label: 'KMS', category: 'security', description: 'Key management service', whatItDoes: 'Creates, stores, and rotates encryption keys used by applications and infrastructure.', usedFor: ['Envelope encryption', 'Token and data key management', 'Key rotation and access control'] },
];
