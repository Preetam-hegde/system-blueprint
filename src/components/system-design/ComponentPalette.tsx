...
export default function ComponentPalette({
  ...
  const onDragStart = useCallback(
    (event: React.DragEvent, nodeType: SystemNodeType) => {
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              className={
                mobile
                  ? "h-10 w-10 rounded-xl border border-border bg-background/90 shadow-lg backdrop-blur mb-1"
                  : "h-8 w-8 mb-2"
              }
              onClick={onToggle}
            >
                  ...
                  <span className="flex items-center gap-2">
                    ...
                    <span className="flex flex-col items-start">
                      <span className="flex items-center gap-1.5">
                        {CATEGORY_LABELS[cat]}
                        <span className="text-[9px] font-normal text-muted-foreground bg-muted rounded px-1">
                          {items.length}
                        </span>
                      </span>
