...
const ICON_MAP: Record<SystemNodeType, React.ElementType> = {
  ...
  "data-lake": Layers,
  "graph-db": GitBranch,
  "time-series-db": ChartNoAxesColumn,
  "message-queue": Mail,
  "event-bus": Radio,
  ...
  "ai-agent": Bot,
  "fine-tuning": GraduationCap,
  "model-router": Route,
  dns: Wifi,
  firewall: ShieldCheck,
  "rate-limiter": Gauge,
  "service-discovery": Radar,
  "service-mesh": ArrowLeftRight,
  "reverse-proxy": Globe,
  "web-client": Monitor,
  "mobile-client": Smartphone,
  "iot-device": CircuitBoard,
  "desktop-client": Laptop,
  "log-aggregator": FileText,
  "metrics-server": BarChart3,
  tracing: GitBranch,
  alerting: Bell,
  "error-tracker": Bug,
  waf: ShieldAlert,
...
function SystemNodeComponent({ data, selected, id }: NodeProps) {
  ...
  const loadPct =
    d.throughputLimit > 0 ? (d.currentLoad / d.throughputLimit) * 100 : 0;
  ...
  const isReplayBottleneck =
    isReplayMode && replayTrace.bottleneckNodeIds.includes(id);
  ...
  return (
    ...
    >
      {isFailed && (
        <div className="absolute inset-0 rounded-xl bg-destructive/10 pointer-events-none" />
      )}
      <Handle
        type="target"
      ...
      <div className="flex items-center gap-2.5 mb-1 pr-8">
        ...
        <div className="flex flex-col min-w-0">
          ...
          <div className="flex items-center gap-1 mt-0.5">
            ...
            <span className="text-[9px] text-muted-foreground opacity-40">
              ·
            </span>
      ...
      {d.currentLoad > 0 && (
        <div className="mt-2">
          ...
          <div className="flex items-center justify-between mt-1">
            ...
            <span
              className={`text-[9px] font-bold ${loadPct > 90 ? "text-destructive" : loadPct > 60 ? "text-amber-500" : "text-muted-foreground"}`}
            >
              {Math.round(loadPct)}%
            </span>
          </div>
