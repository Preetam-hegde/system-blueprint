...
const TEMPLATES: Template[] = [
  {
    ...
    name: "Microservices",
    description:
      "Load balancer → API Gateway → multiple services with DB and cache",
    icon: Globe,
    color: "221 83% 53%",
    rating: "good",
    ...
    build: () => ({
      nodes: [
        ...
        makeNode("t-gw", "api-gateway", "API Gateway", "compute", 400, 250, {
          replicas: 2,
        }),
        makeNode("t-svc1", "server", "User Service", "compute", 150, 400, {
          replicas: 3,
        }),
        makeNode("t-svc2", "server", "Order Service", "compute", 400, 400, {
          replicas: 3,
        }),
        makeNode("t-svc3", "server", "Product Service", "compute", 650, 400, {
          replicas: 2,
        }),
        makeNode("t-cache", "cache", "Redis Cache", "storage", 150, 560, {
          replicas: 2,
        }),
        makeNode("t-db1", "sql-db", "Users DB", "storage", 400, 560, {
          replicas: 2,
        }),
        makeNode("t-db2", "nosql-db", "Products DB", "storage", 650, 560, {
          replicas: 2,
        }),
      ],
      edges: [
    ...
    build: () => ({
      nodes: [
        ...
        makeNode(
          ...
          350,
          270,
          { replicas: 2, throughputLimit: 100 },
        ),
        makeNode("r-cache", "cache", "Prompt Cache", "storage", 100, 420, {
          replicas: 2,
        }),
        makeNode(
          "r-rag",
        ...
        makeNode("r-llm", "llm", "GPT-4", "ai-ml", 350, 570, {
          modelName: "gpt-4",
          contextWindow: 128000,
          throughputLimit: 50,
          latency: 800,
        }),
        makeNode("r-vec", "vector-db", "Pinecone", "storage", 600, 570, {
          storageSize: 50,
        }),
        makeNode(
          "r-obj",
  ...
  },
  {
    ...
    build: () => ({
      nodes: [
        makeNode("ev-api", "api-gateway", "API Gateway", "compute", 350, 0, {
          replicas: 2,
        }),
        makeNode("ev-svc1", "server", "Producer A", "compute", 150, 150),
        makeNode("ev-svc2", "server", "Producer B", "compute", 550, 150),
        makeNode("ev-bus", "event-bus", "Kafka", "messaging", 350, 300, {
          replicas: 3,
        ...
        makeNode("ev-c1", "server", "Consumer A", "compute", 350, 450, {
          replicas: 2,
        }),
        makeNode("ev-c2", "server", "Consumer B", "compute", 550, 450, {
          replicas: 2,
        }),
  ...
  },
  {
    ...
    name: "E-Commerce",
    description:
      "Full e-commerce stack with CDN, payments, search, and notifications",
    icon: ShoppingCart,
    color: "142 71% 45%",
    rating: "good",
    ...
    build: () => ({
      nodes: [
        ...
        makeNode("ec-gw", "api-gateway", "API Gateway", "compute", 350, 260, {
          replicas: 2,
        }),
        ...
        makeNode("ec-cache", "cache", "Catalog Cache", "storage", 600, 550, {
          replicas: 2,
        }),
        makeNode(
          "ec-queue",
        ...
        makeNode("ec-db", "sql-db", "Main DB", "storage", 350, 550, {
          replicas: 2,
        }),
        makeNode(
          "ec-worker",
  ...
  },
  {
    ...
    name: "Real-time Chat",
    description:
      "WebSocket-based chat with presence, history, and push notifications",
    icon: MessageSquare,
    color: "199 89% 48%",
    rating: "good",
    ...
    build: () => ({
      nodes: [
        ...
        makeNode("ch-ws", "server", "WS Server", "compute", 350, 270, {
          replicas: 4,
        }),
        makeNode("ch-pub", "event-bus", "Redis PubSub", "messaging", 150, 400, {
          replicas: 3,
        }),
        makeNode("ch-api", "server", "REST API", "compute", 550, 400, {
          replicas: 2,
        }),
  ...
  },
  {
    ...
    name: "URL Shortener",
    description:
      "Optimized redirect hot path with cache-first lookup, persistent store, and async analytics",
    icon: Link2,
    color: "12 76% 56%",
    rating: "good",
    ...
    build: () => ({
      nodes: [
        ...
        makeNode("us-app", "server", "Short URL Service", "compute", 350, 360, {
          replicas: 3,
          throughputLimit: 4000,
        }),
        makeNode("us-cache", "cache", "Redirect Cache", "storage", 130, 500, {
          replicas: 2,
        }),
        makeNode("us-db", "nosql-db", "URL Store", "storage", 350, 500, {
          replicas: 2,
        }),
        makeNode(
          "us-bus",
  ...
  },
  {
    ...
    name: "Social Feed",
    description:
      "Home feed system with fan-out workers, caches, media storage, and notifications",
    icon: Users,
    color: "338 82% 57%",
    rating: "advanced",
    ...
    build: () => ({
      nodes: [
        ...
        makeNode("sf-gw", "api-gateway", "API Gateway", "compute", 340, 120, {
          replicas: 2,
        }),
        makeNode("sf-feed", "server", "Feed API", "compute", 340, 250, {
          replicas: 4,
        }),
        makeNode("sf-post", "server", "Post Service", "compute", 110, 380, {
          replicas: 3,
        }),
        makeNode("sf-graph", "graph-db", "Social Graph", "storage", 110, 520),
        makeNode("sf-cache", "cache", "Timeline Cache", "storage", 340, 520, {
          replicas: 3,
        }),
        makeNode(
          "sf-fanout",
        ...
        makeNode("sf-feeddb", "nosql-db", "Feed Store", "storage", 340, 660, {
          replicas: 2,
        }),
        makeNode(
          "sf-media",
  ...
  },
  {
    ...
    description:
      "Separated control plane and media plane with CDN-backed delivery and async transcoding",
    icon: Clapperboard,
    color: "205 85% 52%",
    rating: "advanced",
    ...
    build: () => ({
      nodes: [
        ...
        makeNode("vs-gw", "api-gateway", "Streaming API", "compute", 350, 220, {
          replicas: 2,
        }),
        makeNode("vs-meta", "server", "Metadata Service", "compute", 140, 360, {
          replicas: 2,
        }),
        makeNode("vs-upload", "server", "Upload Service", "compute", 560, 360, {
          replicas: 2,
        }),
        makeNode("vs-db", "sql-db", "Video Metadata DB", "storage", 140, 520, {
          replicas: 2,
        }),
        makeNode(
          "vs-obj",
        ...
        makeNode(
          ...
          560,
          660,
          { replicas: 6, throughputLimit: 300 },
        ),
        makeNode(
          "vs-dlq",
          "dead-letter-queue",
  ...
  },
  {
    ...
    name: "Ride Dispatch",
    description:
      "Dispatch and matching with live location ingest, geo cache lookups, and trip event streaming",
    icon: CarTaxiFront,
    color: "145 68% 42%",
    rating: "advanced",
    ...
    build: () => ({
      nodes: [
        ...
        makeNode(
          ...
          340,
          250,
          { replicas: 4, throughputLimit: 2500 },
        ),
        makeNode(
          "rd-location",
          "server",
        ...
        makeNode("rd-trips", "sql-db", "Trips DB", "storage", 340, 520, {
          replicas: 2,
        }),
        makeNode("rd-geo", "cache", "Geo Cache", "storage", 120, 520, {
          replicas: 2,
        }),
        makeNode(
          "rd-metrics",
  ...
  },
  {
    ...
    name: "LLM Gateway",
    description:
      "Prompt API with model routing, caching, vector retrieval, and key management",
    icon: Sparkles,
    color: "272 84% 60%",
    rating: "advanced",
    ...
    build: () => ({
      nodes: [
        ...
        makeNode(
          ...
          350,
          240,
          { replicas: 2, throughputLimit: 300 },
        ),
        makeNode("lg-cache", "cache", "Prompt Cache", "storage", 120, 380, {
          replicas: 2,
        }),
        makeNode(
          "lg-rag",
        ...
        makeNode("lg-vec", "vector-db", "Vector Store", "storage", 580, 380, {
          storageSize: 300,
        }),
        makeNode("lg-llm1", "llm", "Fast Model", "ai-ml", 220, 540, {
          modelName: "fast-model",
          latency: 250,
          throughputLimit: 200,
        }),
        makeNode("lg-llm2", "llm", "Reasoning Model", "ai-ml", 480, 540, {
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-2">
          ...
          <Badge
            variant="secondary"
            className="h-5 shrink-0 rounded-full px-1.5 text-[9px]"
          >
            {TEMPLATES.length}
          </Badge>
        </div>
        <p className="mt-1 text-[11px] leading-4 text-muted-foreground">
          Load a production-style architecture and customize it instead of
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <Tooltip>
        ...
        <TooltipContent side="right">
          Browse interview system design templates
        </TooltipContent>
      </Tooltip>
                ...
                >
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-1.5">
                      ...
                      <Badge
                        variant={
                          t.rating === "advanced" ? "default" : "secondary"
                        }
            ...
            {visibleTemplates.length === 0 && (
              <div className="rounded-xl border border-dashed border-border p-6 text-center md:col-span-2">
                <div className="text-sm font-medium text-foreground">
                  No templates for `{activeTag}`
                </div>
                <div className="mt-1 text-xs text-muted-foreground">
                  Choose another topic filter.
                </div>
              </div>
            )}
          </div>
