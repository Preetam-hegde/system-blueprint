...
export default function CapacityPlannerDialog() {
  ...
  const { nodes, buildCapacityPlan } = useDesignStore(
    useShallow((s) => ({
      nodes: s.nodes,
      buildCapacityPlan: s.buildCapacityPlan,
    })),
  );

  const plan = buildCapacityPlan(growthFactor, trafficSpikeMultiplier);

  const highRiskCount = plan.nodes.filter(
    (node) => node.projectedUtilizationPct >= 90,
  ).length;
  const mediumRiskCount = plan.nodes.filter(
    (node) =>
      node.projectedUtilizationPct >= 70 && node.projectedUtilizationPct < 90,
  ).length;
        ...
        <div className="max-h-[78vh] space-y-4 overflow-y-auto px-4 py-4 sm:px-5 sm:py-5">
          <div className="space-y-4 rounded-xl border border-border p-4">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="secondary">
                  {trafficSpikeMultiplier.toFixed(2)}x
                </Badge>
              </div>
              <Slider
            ...
            <div className="rounded-lg border border-border bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
              Combined what-if multiplier:{" "}
              <span className="font-semibold text-foreground">
                {plan.totalProjectedMultiplier.toFixed(2)}x
              </span>
            </div>
            ...
            <div className="grid gap-3 text-xs text-muted-foreground sm:grid-cols-2">
              <div className="rounded-lg border border-border p-2">
                Current hourly cost:{" "}
                <span className="font-semibold text-foreground">
                  {usd.format(plan.totalHourlyCost)}
                </span>
              </div>
              <div className="rounded-lg border border-border p-2">
                Projected hourly cost:{" "}
                <span className="font-semibold text-foreground">
                  {usd.format(plan.projectedHourlyCost)}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="hidden overflow-hidden rounded-xl border border-border md:block">
            ...
            <div className="max-h-[420px] overflow-y-auto">
              {plan.nodes.map((node) => (
                <div
                  key={node.nodeId}
                  className="grid grid-cols-12 gap-2 px-3 py-2 text-xs border-t border-border/60 items-center"
                >
                  <div className="col-span-3 font-medium truncate">
                    {node.label}
                  </div>
                  <div className="col-span-2 font-mono">
                    {Math.round(node.currentRps)}
                  </div>
                  <div className="col-span-2 font-mono">
                    {Math.round(node.projectedRps)}
                  </div>
                  <div className="col-span-1">{node.replicas}</div>
                  <div className="col-span-1">
                    <Badge
                      variant={
                        node.requiredReplicas > node.replicas
                          ? "destructive"
                          : "secondary"
                      }
                    >
                  ...
                  <div className="col-span-1">
                    <Badge
                      variant={
                        node.projectedUtilizationPct >= 90
                          ? "destructive"
                          : "secondary"
                      }
                      className={
                        node.projectedUtilizationPct >= 70 &&
                        node.projectedUtilizationPct < 90
                  ...
                  <div className="col-span-2 font-mono">
                    {usd.format(node.projectedHourlyCost)}
                  </div>
                </div>
              ))}
            </div>
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 text-[11px] text-muted-foreground">
                      {Math.round(node.currentRps)} now ·{" "}
                      {Math.round(node.projectedRps)} projected
                    </div>
                  </div>
                  <Badge
                    variant={
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                  ...
                  <div className="rounded-lg border border-border px-2 py-1.5">
                    Utilization:{" "}
                    <span className="font-semibold text-foreground">
                      {Math.round(node.projectedUtilizationPct)}%
                    </span>
                  </div>
                  <div className="rounded-lg border border-border px-2 py-1.5">
                    Cost:{" "}
          ...
          <div className="text-xs leading-5 text-muted-foreground">
            Nodes needing scale-up:{" "}
            <span className="font-semibold text-foreground">
              {plan.nodes.filter((n) => n.requiredReplicas > n.replicas).length}
            </span>
            {" · "}
            Bottlenecks if unchanged:{" "}
            <span className="font-semibold text-foreground">
              {plan.projectedBottlenecks}
            </span>
            {" · "}
            High risk:{" "}
            <span className="font-semibold text-foreground">
