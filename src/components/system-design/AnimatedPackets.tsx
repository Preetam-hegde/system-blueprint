...
export default function AnimatedPackets() {
  ...
  useEffect(() => {
    ...
    const activeEdges = edges.filter(
      (e) => !failed.has(e.source) && !failed.has(e.target),
    );
    ...
    const interval = setInterval(
      () => {
        ...
        const touchesLatencyRegion =
          simulation.scenario.type === "regional-latency" &&
          simulation.scenario.region &&
          (sourceData?.region === simulation.scenario.region ||
            targetData?.region === simulation.scenario.region);
        const touchesBacklog =
          simulation.scenario.type === "queue-backlog" &&
          Boolean(
            simulation.scenario.queueNodeIds?.includes(randomEdge.source) ||
            simulation.scenario.queueNodeIds?.includes(randomEdge.target),
          );
